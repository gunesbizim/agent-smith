...
3. For every finding, produce: `{ severity: blocker|suggestion, file, line, problem, fix }`.
   - **blocker** — a concrete security defect that should stop the merge.
   - **suggestion** — a security improvement that does not block on its own.
...
one-line "security verdict"). Do NOT synthesize across lenses — that is the orchestrator's job.
