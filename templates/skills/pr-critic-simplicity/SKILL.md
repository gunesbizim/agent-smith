...
3. For every finding, produce: `{ severity: blocker|suggestion, file, line, problem, fix }`.
   - **blocker** — a concrete simplicity defect that should stop the merge.
   - **suggestion** — a simplicity improvement that does not block on its own.
...
one-line "simplicity verdict"). Do NOT synthesize across lenses — that is the orchestrator's job.
