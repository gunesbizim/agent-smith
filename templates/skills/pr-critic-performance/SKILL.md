...
3. For every finding, produce: `{ severity: blocker|suggestion, file, line, problem, fix }`.
   - **blocker** — a concrete performance defect that should stop the merge.
   - **suggestion** — a performance improvement that does not block on its own.
...
one-line "performance verdict"). Do NOT synthesize across lenses — that is the orchestrator's job.
