...
export async function initCommand(opts: InitOptions): Promise<void> {
  ...
  // Step 8b — LLM-author skills grounded in the architecture docs + real code.
  // On by default when claude is present; --no-llm or --dry-run skips it (templates stay).
  if (opts.llm !== false && !opts.dryRun) {
    const skillGenSpinner = ora("Generating skills with Claude (understands project, fans out subagents)...").start();
    const result = generateSkills(targetDir);
    if (result.ran) {
      const summarySuffix = result.summary ? ` — ${result.summary}` : "";
      skillGenSpinner.succeed(`Skills generated by Claude${summarySuffix}`);
    } else {
