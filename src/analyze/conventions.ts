...
const TEST_FIXTURE_FILES = new Set(["conftest.py", "factories.py", "factory.py"]);
...
function detectLayeredArchitecture(dirs: Set<string>): Convention | null {
  const layerHits = Object.entries(LAYER_DIRS)
    .map(([layer, names]) => ({ layer, dir: hasAnyDir(dirs, names) }))
    .filter((h) => h.dir);
  if (layerHits.length < 2) return null;
  return {
...
function detectTypedBoundaries(dirs: Set<string>): Convention | null {
  const boundaryDir = hasAnyDir(dirs, TYPED_BOUNDARY_DIRS);
  if (!boundaryDir) return null;
  return {
...
function detectStructuredLogging(project: DetectedProject): Convention | null {
  if (project.backend?.loggingPattern !== "structured") return null;
  return {
...
function detectTestSuite(dirs: Set<string>, files: string[]): Convention | null {
  const testDir = hasAnyDir(dirs, TEST_DIRS);
  const fixtureFile = files.find((f) => TEST_FIXTURE_FILES.has(path.basename(f)));
  if (!testDir && !fixtureFile) return null;
...
function detectEnforcedAuth(project: DetectedProject): Convention | null {
  const auth = project.backend?.authMethod;
  if (!auth || auth === "none" || auth === "unknown") return null;
  return {
    id: "enforced-auth",
...
export async function detectConventions(rootPath: string, project: DetectedProject): Promise<Convention[]> {
  ...
  const candidates = [
    detectLayeredArchitecture(dirs),
    detectTypedBoundaries(dirs),
    detectStructuredLogging(project),
    detectTestSuite(dirs, files),
    detectEnforcedAuth(project),
  ];

  return candidates.filter((c): c is Convention => c !== null);
}
